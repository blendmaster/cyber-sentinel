...
public class ContextGraphBuilder {
	private static Logger logger = Logger.getLogger(ContextGraphBuilder.class.getName());
	...
	public ContextGraph build(List<Message> messages) {
		...
		for(Message message : messages) {
			try {
				Terms messageTerms = reader.getTermVector(message.getDocID(), "contents");
				if(messageTerms == null) continue;
			...
			try {
				...
				while((next = e.next()) != null) {
					...
					while((next2 = e2.next()) != null) {
						Term term2 = new Term("contents", BytesRef.deepCopyOf(next2));
						if(!term1.equals(term2)) {
							Entry<Term, Term> terms = new AbstractMap.SimpleEntry<>(term1, term2);
